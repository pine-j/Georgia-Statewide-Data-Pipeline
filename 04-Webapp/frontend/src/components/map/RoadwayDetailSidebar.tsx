...
const ATTRIBUTE_LABELS: Record<string, AttributeMeta> = {
  ...
  AADT: { label: "AADT", description: "Annual Average Daily Traffic — total vehicles per day averaged over the year (2024). Sourced from the GDOT state 2024 dataset and HPMS federal submission, with limited interpolation for the remaining ~3.5% of segments." },
  AADT_2024_OFFICIAL: { label: "AADT (State 2024)", description: "Direct match against the GDOT state 2024 traffic GDB, before any pipeline-derived fill. Audit trail for state-system records." },
  AADT_2024_HPMS: { label: "AADT (HPMS)", description: "Raw 2024 AADT from the GDOT HPMS federal submission. Shown alongside the state 2024 GDB value for cross-validation." },
  AADT_2024_SOURCE: { label: "AADT Source", description: "Source of the 2024 AADT value: GDOT state dataset (`official_exact`), GDOT HPMS federal submission (`hpms_2024`), direction mirror (INC↔DEC sibling), analytical gap-fill (interpolation), nearest neighbor (closest covered segment on the same route), or missing." },
  AADT_2024_CONFIDENCE: { label: "AADT Confidence", description: "Confidence tier for the 2024 AADT value. high — Actual measurement or both GDOT sources agree; medium — Estimated/Calculated by GDOT, or single-source official; low — pipeline-derived (mirror/interpolation/nearest) or the two GDOT sources disagree; missing — no value available." },
  AADT_2024_SOURCE_AGREEMENT: { label: "Source Agreement", description: "Cross-check between the state 2024 GDB and the HPMS federal submission: state_only, hpms_only, both_agree (within ±15% or ±200 veh/day), both_disagree." },
  AADT_2024_STATS_TYPE: { label: "Measurement Type", description: "GDOT Statistics_Type: Actual (measured this year), Estimated (factored from older counts), or Calculated (derived)." },
  AADT_2024_SAMPLE_STATUS: { label: "Sample Status", description: "GDOT SampleStatus describing the timing or nature of the most recent sample-adequacy pass." },
  AADT_2024_FILL_METHOD: { label: "Fill Method", description: "Internal label describing how the AADT value was populated when no direct state match was available." },
...
const SECTIONS: AttributeSection[] = [
  {
    title: "Traffic",
    keys: new Set(["AADT", "FUTURE_AADT_2044", "TRUCK_AADT", "TRUCK_PCT", "PCT_SADT", "PCT_CADT", "K_FACTOR", "D_FACTOR", "VMT", "AADT_2024_OFFICIAL", "AADT_2024_HPMS", "AADT_2024_SOURCE_AGREEMENT", "AADT_2024_STATS_TYPE", "AADT_2024_SAMPLE_STATUS", "AADT_2024_SOURCE", "AADT_2024_CONFIDENCE", "AADT_2024_FILL_METHOD"]),
  },
  {
