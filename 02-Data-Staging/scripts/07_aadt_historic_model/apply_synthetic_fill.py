"""IDW fill for 2020/2024 FC 6-7 segments flagged as synthetic HPMS defaults.

Writes AADT_{2020,2024}_LOCAL_FILL + _LOCAL_FILL_CONFIDENCE alongside the
existing HPMS columns — does not replace them.
"""
...
def main() -> None:
    ...
    for year in [2020, 2024]:
        ...
        target_uids = pd.read_sql(
            f"SELECT unique_id FROM segments WHERE AADT_{year}_HPMS_SYNTHETIC = 1",
            conn,
        )["unique_id"].tolist()
        logger.info("  %d synthetic-flagged segments", len(target_uids))
        ...
        knn = knn[knn["unique_id"].isin(target_set)]
    ...
    logger.info("\nTotal segments: %d (unchanged)", seg_count)
    conn.close()
    logger.info("Done.")
