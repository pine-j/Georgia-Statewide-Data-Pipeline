...
**Consumer guidance — 2020 and 2024 HPMS contain FHWA volumegroup-default fills on FC 6-7.** 60.3% of non-null `AADT_2020_HPMS` values (150,566 / 249,604) and 62.5% of `AADT_2024_HPMS` values (158,813 / 254,208) concentrate in a small set of mass-repeat integers — for example 47,635 segments share the value `1662` in 2020 and 48,178 share `2250` in 2024, with 99.9%+ of each cluster on FC 7. These are FHWA-assigned volumegroup **default fills** for non-sampled FC 6-7 segments: the same sampling regime that leaves FC 6-7 NULL in 2022/2023 substitutes a state-level default integer in 2020/2024. FC 1-5 is not affected — the mass-repeat concentration is confined to FC 7 (and to a lesser extent FC 6). Downstream consumers must treat mass-repeat values on FC 6-7 in 2020 and 2024 as "synthetic defaults, not measurements" for any analysis that relies on per-segment empirical AADT (time-series trend detection, per-segment growth rates, AADT modeling). The canonical 2024 `AADT` column is unaffected — it is sourced from the GDOT 2024 traffic GDB, not HPMS — and the two-source disagreement columns (`AADT_2024_SOURCE_AGREEMENT`) already surface segments where the HPMS value disagrees with the state source. The AADT modeling v2 plan (`C:/Users/adith/.claude/plans/aadt-modeling-scoped-2020-2024-v2.md`) stages `AADT_{year}_HPMS_SYNTHETIC` flag columns on `segments` and pivots training targets away from HPMS onto GDOT station-level Actual counts for precisely this reason. Evidence: `02-Data-Staging/docs/aadt_hpms_historic_coverage.md §Synthetic default-fills on FC 6-7 in 2020 and 2024`.
