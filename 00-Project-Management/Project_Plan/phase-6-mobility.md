# Phase 6 — Mobility (RAPTOR Category — Weight: 0.20)
...
> - **Historic actual AADT only**: when we ingest GDOT route-segment history, we keep the actual AADT series and drop legacy `Future_AADT` / `FUTURE_AAD` projections. The only canonical future projection in the roadway network is the current 2024 GDOT record.
...
### 6.4 NPMRDS (National Performance Management Research Data Set)
- **Source**: FHWA / RITIS
...
### 6.5 ARC Historical Traffic Counts (2008-2017)
...
### 6.8 GDOT Traffic_Historical.zip (Statewide)
...
### 6.9 GDOT 2010-2019 Published Traffic
- **Source**: GDOT file server
...
### 6.10 HPMS Georgia Shapefiles (Yearly, 2011-2023)
- **Source**: FHWA HPMS
...
### 6.6 Georgia Traffic Monitoring Program Guide
- **Source**: Georgia DOT
...
### 6.7 GEOCOUNTS — Georgia Traffic Counts
- **Source**: GEOCOUNTS
...
**File**: `05-RAPTOR-Integration/states/Georgia/categories/Mobility.py`

**AADT**: From base roadway inventory (already loaded in Phase 1). Future 2050 via growth factor computed from historic actual AADT trend data only. Do not use legacy projected future-AADT values from historic GDOT segment files; the canonical future projection is `FUTURE_AADT` from the 2024 traffic record.
